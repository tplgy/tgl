...
std::shared_ptr<tgl_channel> tglf_fetch_alloc_channel (struct tl_ds_chat *DS_C) {
  ...
  tgl_state::instance()->callback()->channel_update(tgl_get_peer_id(C->id), *DS_C->access_hash, *(DS_C->date), std::string(DS_C->title->data, DS_C->title->len), std::string(DS_C->username->data, DS_C->username->len));
  tgl_state::instance()->callback()->avatar_update(tgl_get_peer_id(C->id), C->id.peer_type, C->photo_big, C->photo_small);
...
std::shared_ptr<tgl_message> tglf_fetch_encrypted_message (struct tl_ds_encrypted_message *DS_EM) {
  ...
  if (*in_ptr == CODE_decrypted_message_layer) {
    ...
    // FIXME: We need to queue any holes found and re-order all messages by the sequence number.
...
std::shared_ptr<tgl_message> tglf_fetch_alloc_encrypted_message (struct tl_ds_encrypted_message *DS_EM) {
  std::shared_ptr<tgl_message> M = tglf_fetch_encrypted_message (DS_EM);
  if (!M) { return M; }
